...
class App extends React.Component {
	...
	filter(users, filter) {
		...
		// эта часть сделана не правильно
		// тут речь шла о сортировке скорее, не о фильтрах.
		// но как не назови работает странно
		// при каждом клике получаем таблицу отсортированную по одному из столбцов от меньшего к большему
		// при этом отсортировать по тому же столбцу от большего к меньшему нет
		// фильтр по дате рождения (кнопка Дата регистрации) получается и вовсе не работает
		// эта кнопка передает сюда строку 'id' и код ниже возвращает массив ничего с ним не делая
